...
function getLastAssistantApiError(messages: Array<AssistantLoopMessage>): string | null {
    for (let i = messages.length - 1; i >= 0; i--) {
        const message = messages[i];
        if (message?.role === "assistant") {
            if (message.stopReason !== "error") return null;
            return message.errorMessage?.trim() || "Unknown AI API error";
        }
    }
    return null;
}

function formatApiErrorForUser(errorMessage: string): string {
    const compactError = errorMessage.replace(/\s+/g, " ").trim();
    if (!compactError) return "Unknown AI API error";

    if (/array too long|input too long/i.test(compactError)) return "input too long";

    const noApiKeyMatch = compactError.match(/No API key for provider:\s*([^\s]+)/i);
    if (noApiKeyMatch) {
        return `No API key for provider: ${noApiKeyMatch[1]}`;
    }

    if (/request timed out/i.test(compactError)) return "Request timed out";
    if (/connection error/i.test(compactError)) return "Connection error";

    const openAiMatch = compactError.match(/OpenAI API error \((\d+)\):\s*(.*)/i);
    if (openAiMatch) {
        const [, statusCode, remainder] = openAiMatch;
        if (statusCode === "502") return "502 upstream server error";

        const withoutHtml = remainder.split(/<!DOCTYPE html>/i)[0]?.trim() || "";
        const withoutDuplicateStatus = withoutHtml.replace(new RegExp(`^${statusCode}\\s*`), "").trim();
        const shortMessage = `${statusCode} ${withoutDuplicateStatus || "API error"}`.trim();
        return shortMessage.length > 120 ? `${shortMessage.slice(0, 117)}...` : shortMessage;
    }

    return compactError.length > 120 ? `${compactError.slice(0, 117)}...` : compactError;
}
...
export default function loopExtension(pi: ExtensionAPI): void {
    ...
    function wasLastAssistantAborted(messages: Array<AssistantLoopMessage>): boolean {
        for (let i = messages.length - 1; i >= 0; i--) {
            const message = messages[i];
            if (message?.role === "assistant") {
    ...
    pi.on("agent_end", async (event, ctx) => {
        ...
        const apiError = getLastAssistantApiError(event.messages);
        if (apiError) {
            const retries = (loopState.apiErrorRetries ?? 0) + 1;
            loopState = { ...loopState, apiErrorRetries: retries };
            persistState(loopState);
            updateStatus(ctx, loopState);

            const visibleError = formatApiErrorForUser(apiError);
            if (retries > MAX_API_ERROR_RETRIES) {
                clearLoopState(ctx);
                if (ctx.hasUI) {
                    ctx.ui.notify(
                        `Loop stopped after ${MAX_API_ERROR_RETRIES} AI API retries: ${visibleError}`,
                        "warning",
                    );
                }
                return;
            ...
            if (ctx.hasUI) {
                ctx.ui.notify(
                    `AI API error, retry ${retries}/${MAX_API_ERROR_RETRIES}: ${visibleError}`,
                    "warning",
                );
            }
            triggerLoopPrompt(ctx);
        ...
        if ((loopState.apiErrorRetries ?? 0) !== 0) {
            loopState = { ...loopState, apiErrorRetries: 0 };
            persistState(loopState);
            updateStatus(ctx, loopState);
        }
